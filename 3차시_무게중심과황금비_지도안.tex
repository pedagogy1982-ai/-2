...
\textbf{탐구 활동 (12분)} --- 삼각형 A$(0,0)$, B$(6,0)$, C$(3,9)$의 무게중심을 구한 뒤, 각 변을 $2:1$로 내분한 세 점으로 만든 새 삼각형의 무게중심을 다시 구해 두 값이 일치하는지 확인 (모둠 활동, 결과를 칠판에 취합).\newline
...
\begin{answerbox}
삼각형 A$(0,0)$, B$(6,0)$, C$(3,9)$의 무게중심: $G=\left(\dfrac{0+6+3}{3},\dfrac{0+0+9}{3}\right)=(3,3)$

\vspace{4pt}
각 변을 $2:1$로 내분한 점 (A$\to$B, B$\to$C, C$\to$A 방향): P$(4,0)$, Q$(4,6)$, R$(1,3)$

\vspace{4pt}
삼각형 PQR의 무게중심: $\left(\dfrac{4+4+1}{3},\dfrac{0+6+3}{3}\right)=(3,3)$

\textbf{\color{goldstrong}$\to$ 두 무게중심이 $(3,3)$으로 정확히 일치한다.}
\end{answerbox}
