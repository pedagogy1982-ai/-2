...
% ---------- 줄쳐진 기록 공간 (실제 cm 단위, 0.7cm 줄간격) ----------
% #1 = 높이(cm)
\newcommand{\writespace}[1]{%
  \begin{tikzpicture}
    \useasboundingbox (0,0) rectangle (\linewidth,-#1 cm);
    \draw[writeline, rounded corners=2pt] (0,0) rectangle (\linewidth,-#1 cm);
    \foreach \y in {0.7,1.4,...,#1} {
      \draw[writeline, dash pattern=on 2pt off 2pt] (0.15,-\y cm) -- (\linewidth-0.15cm,-\y cm);
    }
  \end{tikzpicture}
}

\newcommand{\fillblank}[1]{\makebox[#1]{\dotfill}}
...
\end{tabularx}
\noindent
\begin{minipage}[t]{0.32\linewidth}\writespace{2.6}\end{minipage}\hfill
\begin{minipage}[t]{0.32\linewidth}\writespace{2.6}\end{minipage}\hfill
\begin{minipage}[t]{0.32\linewidth}\writespace{2.6}\end{minipage}
